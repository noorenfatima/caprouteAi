...
def train_model(config_path: str = "capital_routing_config.json") -> Dict:
    ...
    print(f"   [ML] {len(df)} samples generated across {df['_origin'].nunique()} origins x {df['_zone'].nunique()} zones x {df['_purpose'].nunique()} purposes")
    ...
    print(f"   RF  -> MAE: {rf_mae:.3f} pts | R2: {rf_r2:.4f}")

    # ── Gradient Boosting ─────────────────────────────────────────────────────
    print("\n[ML] Training Gradient Boosting (XGBoost-style)...")
    ...
    print(f"   GB  -> MAE: {gb_mae:.3f} pts | R2: {gb_r2:.4f}")
    ...
    print(f"\n[ML] Ensemble -> MAE: {ens_mae:.3f} pts | R2: {ens_r2:.4f}")
...
if __name__ == "__main__":
    ...
    print("\n[ML] Training complete. Use FrictionScorer('friction_model.pkl') in logic.py")
